  ...
  <div class="postsContainer">
    ...
    <main>
        <div>
          ...
          $querySQL = "SELECT postID, email, post, dateCreated, comments FROM neonPosts ORDER BY dateCreated DESC";
          $queryResult = $dbConn->query($querySQL);
          ...
          else
          {
            while($rowObj = $queryResult->fetchObject())
            {
              echo "<div class='posts'><p>{$rowObj->email}: <hr>{$rowObj->post} <hr>Posted At: {$rowObj->dateCreated} <a href='comments.php?postID={$rowObj->postID}'>Comments:</a>   {$rowObj->comments}</p></div>";
            }
          }
          ?>
        </div>
